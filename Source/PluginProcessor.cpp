...
void Project13AudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    spec.numChannels = 1;       // if working with stereo  getTotalNumInputChannels();
    ...
    Project13AudioProcessor::createParameterLayout()
{
        ...
        /*
         ...
         drive: 1 - 100
         */
        // ladderFilter mode: LadderFilterMode enum (int)
        ...
        layout.add(std::make_unique<juce::AudioParameterChoice>(
               ...
               choices,
               0));
        // general filter freq: 20hz - 20,000hz (20Khz) in 1hz steps
        ...
        layout.add(std::make_unique<juce::AudioParameterFloat>(
               ...
               750.f,
               "Hz"));
        //general filter Q: 0.1 - 10 in 0.5 steps
        ...
        //general filter gain: -24db to +24db in 0.5db increments
...
void Project13AudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // [DONE]: bypass for each DSP element
    ...
    // TODO: snap dropped tabs to the correct position
...
//    rightChannel.process(block.getSingleChannelBlock(1), dspOrder);
    ...
    {
        /*
         figue out how many samples to actually process.
         i.e., you might have a buffer size of 72.
         The first time through this loop sampleToProcess will be 64, becausse maxSampleToProcess is set
             to 64, and samplesRemaining is 72.
         the 2nd time this loop runs, sampleToProcess will be 8, because the previous loop consumed 64
             of the 72 samples.
         */
        auto samplesToProcess = juce::jmin(samplesRemaining, maxSamplesToProcess);   // (4)
        // advanced each smoother 'samplesToProcess' samples
...
void Project13AudioProcessor::MonoChannelDSP::process(juce::dsp::AudioBlock<float> block,
                                                      const DSP_Order& dspOrder)
{
